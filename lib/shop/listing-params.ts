...
/** Map a product list row (API or server query) to shop card props. */
export function mapListItemToShopProduct(row: SerializedListItem): ShopProduct {
  ...
  const availabilityStatus = row.availabilityStatus ?? "In Stock";
  const productAvailability = { stock, availabilityStatus };
  return {
    id: row.id,
    title: row.title,
    ...
    badge,
    stock,
    minimumOrderQuantity: normalizeMinimumOrderQuantity(row.minimumOrderQuantity),
